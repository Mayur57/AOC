...
const calculatePresses = constants => {
  ...
  const X = (c4 * a - c3 * b) / determinant
  const Y = (-c2 * a + c1 * b) / determinant
  return [X, Y]
}
...
function calculatePrizeCost(block) {
  ...
  const [pressesForA, pressesForB] = calculatePresses({ c1, c2, c3, c4, a, b })
